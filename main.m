...
state.stateBounds = [-pi/4, pi/4; -2, 2];
state.numStates = 15;
...
% Set of actions
A = [-100, -50, -1, 0, 1, 50, 100];

% Set of states
S = [linspace(-pi/4, pi/4, state.numStates); linspace(-2, 2, state.numStates)];
...
for i = 2:maxIterations
    ...
    for i = 1:length(Policy)
        if s(1,1) == Policy(1,i) && s(2,1) == Policy(2,i) 
            u = Policy(3, i);
            break;
        end
    end
